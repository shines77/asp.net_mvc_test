...
namespace lolobcn.Depends
{
    ...
    /// 
    /// 来自: http:// www.cnblogs.com/joeylee/archive/2012/10/19/2731366.html
    /// </summary>
    public class MySqlHelper2
    {
        ...
        private static void PrepareCommand(MySqlConnection conn, MySqlTransaction trans, MySqlCommand cmd, CommandType cmdType, string cmdText, MySqlParameter[] cmdParms)
        {
            ...
            // 不会浪费，因为通过SqlDataAdapter的时候，我们都用using来封装了conn的 => using (MySqlConnection conn = new MySqlConnection(connString))
        ...
        /// </summary>
        /// <param name="connString">连接字符串，可为null，如果为null则使用web.config的值，不为null则使用传入的值</param>
